...
class MenuInsert(tk.Toplevel):
    ...
    def setup_UI(self):
        lab_message = tk.Label(self, text='插入新项至:{}(评分：0-100分)'.format(self.table_now), font=(data.DISPLAY_FONT, 15))
        lab_message.grid(row=0, column=0, columnspan=2, sticky=tk.W)

        # 输入提示
        frame_lab = tk.Frame(self)
        lab_NAME = tk.Label(frame_lab, text='名称：', font=(data.DISPLAY_FONT, 12))
        ...
        entry_NAME = tk.Entry(frame_entry, font=(data.DISPLAY_FONT, 12), textvariable=self.v_NAME)
        ...
        frame_button.grid(row=9, column=0, columnspan=2, sticky=tk.E)
